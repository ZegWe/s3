...
function Player:initialize(_parent)
    ...
    self.bucket = UIObject:new("Bucket", PlayerImage.Bucket , self.obj, Vector2(90, 70), Vector2(-170, 300))
    self.bucket:SetClickFunc(function()
        self.bucket:SetVisible(false)
        GameManager.PlaceItem("Bucket")
        self.scene:Tip("已放置【水桶】", 5)
    end)
    self.canvas = UIObject:new("Canvas", PlayerImage.Canvas, self.obj, Vector2(160, 180), Vector2(-30, 300))
    self.canvas:SetClickFunc(function()
        self.canvas:SetVisible(false)
        GameManager.PlaceItem("Canvas")
        self.scene:Tip("已放置【空画布】", 5)
    end)
    self.pigment = UIObject:new("Pigment", PlayerImage.Pigment, self.obj, Vector2(70, 40), Vector2(100, 300))
